...
exports.createCheckoutSession = async (req, res) => {
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `https://triphog.net/success/${req.body.token}/${
        req.body.plan
      }/${Number(req.body.amount)}?session_id={CHECKOUT_SESSION_ID}`,

      cancel_url: "https://triphog.net/cancel",
    });
  ...
  try {
    ...
    if (!admin) {
      ...
      if (admin.password == currentPassword) {
        console.log("Has Matched Admin Password");
...
exports.getUsersForChat = async (req, res) => {
  try {
    ...
    const allUsersToChat = [...superAdmins, ...users];
